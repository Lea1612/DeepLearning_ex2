...
if __name__ == '__main__':
    ...
    parse = Parse(vocab_file=emb_voc, word_vectors_file=emb_voc_vec, has_sub_word_information=False, is_embedded=True,
                  lower_case=True)
    tagger2 = Tagger2(corpus == 'pos')
    ...
    network = network.train_nn(network,
                               data,
                               tag,
                               batch_size=tagger2.batch,
                               validation_data=val_data,
                               validation_tag=val_tag,
                               learning_rate=tagger2.learning_rate,
                               is_pos=corpus == 'pos',
                               epoch=tagger2.epoch,
                               corpus=corpus,
                               parser=parse,
    ...
    network.predict_test(tagger2.test_file, 'test3.' + corpus, parse, network, tagger2.batch)
